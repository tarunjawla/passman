...
impl VaultStorage {
    ...
    pub fn new(vault_name: &str) -> Result<Self> {
        ...
        fs::create_dir_all(&vault_dir).map_err(|e| {
            PassManError::StorageError(format!("Failed to create vault directory: {}", e))
        })?;

        fs::create_dir_all(&backup_dir).map_err(|e| {
            PassManError::StorageError(format!("Failed to create backup directory: {}", e))
        })?;
    ...
    fn get_vault_directory() -> Result<PathBuf> {
        let config_dir = dirs::config_dir().ok_or_else(|| {
            PassManError::StorageError("Cannot determine config directory".to_string())
        })?;
    ...
    pub fn save_vault(&self, vault: &Vault, crypto: &CryptoManager) -> Result<()> {
        ...
        let vault_json =
            serde_json::to_string_pretty(vault).map_err(|e| PassManError::SerializationError(e))?;

        // Encrypt the vault data
        let encrypted_data = crypto.encrypt(vault_json.as_bytes())?;

        // Get the salt used for encryption
        let salt = crypto.get_salt().ok_or_else(|| {
            PassManError::StorageError("No salt available for storage".to_string())
        })?;
        ...
        let temp_path = self.vault_path.with_extension("tmp");
        {
            let mut file = File::create(&temp_path).map_err(|e| {
                PassManError::StorageError(format!("Failed to create temp file: {}", e))
            })?;
            ...
            file.write_all(&encrypted_data).map_err(|e| {
                PassManError::StorageError(format!("Failed to write vault data: {}", e))
            })?;

            file.sync_all().map_err(|e| {
                PassManError::StorageError(format!("Failed to sync vault data: {}", e))
            })?;
        }
    ...
    pub fn load_vault(&self, master_password: &str) -> Result<Vault> {
        if !self.vault_exists() {
            return Err(PassManError::VaultNotFound(format!(
                "Vault not found at: {}",
        ...
        if file_data.len() < 16 {
            return Err(PassManError::StorageError(
                "Vault file is corrupted: too small".to_string(),
            ));
        }

        let salt_bytes: [u8; 16] = file_data[0..16].try_into().map_err(|_| {
            PassManError::StorageError("Failed to read salt from vault file".to_string())
        })?;
    ...
    pub fn export_vault(
        ...
        let mut file = File::create(export_path).map_err(|e| {
            PassManError::StorageError(format!("Failed to create export file: {}", e))
        })?;

        file.write_all(&encrypted_data).map_err(|e| {
            PassManError::StorageError(format!("Failed to write export data: {}", e))
        })?;

        file.sync_all().map_err(|e| {
            PassManError::StorageError(format!("Failed to sync export data: {}", e))
        })?;
    ...
    pub fn import_vault(&self, crypto: &CryptoManager, import_path: &Path) -> Result<Vault> {
        if !import_path.exists() {
            return Err(PassManError::StorageError(format!(
        ...
        let mut file = File::open(import_path).map_err(|e| {
            PassManError::StorageError(format!("Failed to open import file: {}", e))
        })?;

        let mut encrypted_data = Vec::new();
        file.read_to_end(&mut encrypted_data).map_err(|e| {
            PassManError::StorageError(format!("Failed to read import file: {}", e))
        })?;
    ...
    pub fn vault_size(&self) -> Result<u64> {
        ...
        let metadata = fs::metadata(&self.vault_path).map_err(|e| {
            PassManError::StorageError(format!("Failed to get vault metadata: {}", e))
        })?;
    ...
    fn cleanup_old_backups(&self) -> Result<()> {
        let mut backup_files: Vec<_> = fs::read_dir(&self.backup_dir)?
            ...
            .filter(|entry| entry.path().extension().map_or(false, |ext| ext == "vault"))
            .collect();
    ...
    pub fn delete_vault(vault_name: &str) -> Result<()> {
        ...
        if vault_path.exists() {
            fs::remove_file(&vault_path).map_err(|e| {
                PassManError::StorageError(format!("Failed to delete vault: {}", e))
            })?;
        }
...
mod tests {
    ...
    use crate::models::{Account, AccountType, Vault};
